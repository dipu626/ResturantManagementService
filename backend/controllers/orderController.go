...
func GetOrders() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprint("error occured while listening order items")
...
func GetOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprint("error occured while listening order item for order_id" + orderId)
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func CreateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, err := orderCollection.InsertOne(
			ctx,
			order,
		)
...
func UpdateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		orderId := c.Param("order_id")
		if err := c.BindJSON(&order); err != nil {
			//msg := fmt.Sprint("Error to bind to order")
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		if order.Table_id != nil {
			...
			defer cancel()
			if err != nil {
				msg := fmt.Sprint("message: Table_id not found")
				c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
				return
			}
